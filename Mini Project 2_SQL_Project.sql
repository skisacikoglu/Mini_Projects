...
SELECT COUNT( name ) AS no_fee_facilities
FROM Facilities
WHERE membercost = 0
...
SELECT bf.name AS facility_name, CONCAT(m.firstname,' ',m.surname) AS member_name, bf.cost
FROM Members m
JOIN(SELECT b.memid, f.name,
CASE WHEN b.memid = 0 THEN b.slots*f.guestcost ELSE b.slots*f.membercost END AS cost
FROM Facilities f
JOIN(SELECT facid, memid, slots
FROM Bookings
